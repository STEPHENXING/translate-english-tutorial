...
description: 英文视频翻译为中文配音视频的全自动流水线。输入英文视频/音频，输出中文配音视频。使用 faster-whisper 提取英文字幕，VoxCPM2 音色克隆生成中文语音，ffmpeg 视频变速同步合成。当用户要求翻译英文视频、给视频配中文音、视频翻译配音时触发此技能。
---
...
| 参考音频截取 | 从用户提供的参考音频开头截取 12 秒，再转为 VoxCPM2 使用的 16kHz 单声道 WAV |
...
- 流程开始时先向用户确认：这次视频是否是 Andrew Wommack（用户也可能写作 Andrew Walmack）本人的视频？
  ...
  - 如果不是，先让用户提供新的参考音频路径，再继续 Step 3。
...
2. 从参考音频开头截取 12 秒，并规范化为 16kHz 单声道 WAV，保存到 `output-codex/reference_voice.wav`
...
- 参考音频：从用户提供的参考音频开头截取 12 秒并规范化为 16kHz 单声道 WAV；不要从原视频里截取参考音频
